    ...
    <div className="app">
      <header>
        ...
        <nav>
          ...
          <button className={tab === "editorial" ? "active" : ""} onClick={() => setTab("editorial")}>Editorial Studio</button>
...
const EDITORIAL_PLATFORMS = [
  "linkedin", "facebook", "instagram", "x", "newsletter",
  "youtube_shorts", "youtube_long", "podcast",
  "quotes", "hooks", "questions", "predictions",
];

function EditorialStudio({ user, setError, onRepurpose }) {
  const [essay, setEssay] = useState("");
  const [platforms, setPlatforms] = useState(["linkedin", "facebook", "instagram", "x", "quotes", "hooks"]);
  const [brands, setBrands] = useState([]);
  const [brandId, setBrandId] = useState(null);
  const [maxIdeas, setMaxIdeas] = useState(8);
  const [runMultiplier, setRunMultiplier] = useState(false);
  const [result, setResult] = useState(null);
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState("");
  ...
  async function run() {
    if (!essay.trim()) {
      setErr("Paste an essay or long text to run the engine.");
      return;
    }
    setBusy(true);
    setErr("");
  ...
  async function saveAsset(asset, versions) {
    try {
      const updated = await api.editorialUpdateAsset(asset.id, versions);
      const v = updated.versions || versions;
      setResult((r) => ({
        ...r,
        assets: r.assets.map((a) => (a.id === asset.id ? { ...a, versions: v } : a)),
      }));
      return v;
    } catch (e) {
      setErr(e.message);
  ...
  const groupByPlatform = (list) => {
    const m = {};
    for (const a of list) (m[a.platform] ||= []).push(a);
    return m;
  };

  // Build a single Markdown document from the whole run (editable versions included).
  function buildMarkdown() {
    const esc = (s) => (s || "").toString();
    const lines = [];
    lines.push(`# Editorial Studio export`);
    lines.push("");
    lines.push(`- Brand: ${esc(result.brand?.name || "Unknown")}`);
    if (result.brand?.link_url) lines.push(`- Link: ${esc(result.brand.link_url)}`);
    lines.push(`- Ideas mined: ${result.ideas_count} · selected: ${result.selected_ideas} · assets: ${result.assets.length}`);
    lines.push("");
    if (result.multiplier_ip?.length > 0) {
      lines.push(`## New IP to spin up next`);
      for (const ip of result.multiplier_ip) {
        lines.push(`- **${esc(ip.title)}** (${esc(ip.format)}) — ${esc(ip.why)}`);
      }
      lines.push("");
    }
    const byPlatform = groupByPlatform(result.assets);
    for (const [platform, list] of Object.entries(byPlatform)) {
      lines.push(`## ${platform.replace("_", " ")}`);
      lines.push("");
      for (const a of list) {
        const versions = a.versions || [];
        lines.push(`### ${esc(a.kind)} · from idea: ${esc(a.idea_ref)}  (★ ${a.quality_score}/10)`);
        lines.push("");
        versions.forEach((v, i) => {
          lines.push(`**Version ${i + 1}**`);
          lines.push("");
          lines.push(esc(v).trim());
          lines.push("");
        });
      }
    }
  ...
  function downloadMarkdown() {
    ...
    a.href = url;
    a.download = `editorial-export-${Date.now()}.md`;
  ...
  function downloadPdf() {
    // Dependency-free: open a print-optimized window and trigger the browser's
    // "Save as PDF" dialog. No PDF library required.
    const md = buildMarkdown()
      .replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Editorial export</title>
      <style>
        body{font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:780px;margin:32px auto;padding:0 20px;line-height:1.55;color:#111;}
        h1{font-size:26px;} h2{font-size:20px;border-bottom:1px solid #eee;padding-bottom:4px;margin-top:28px;}
        h3{font-size:15px;color:#444;margin-top:18px;}
        pre{white-space:pre-wrap;font-family:inherit;}
        @media print{body{margin:0;}}
      </style></head><body><pre>${md}</pre>
      <script>window.onload=function(){window.print();}</script></body></html>`;
    const w = window.open("", "_blank");
    if (!w) {
      setErr("Pop-up blocked. Allow pop-ups to export PDF, or use the .md download.");
      return;
    }
    w.document.open();
  ...
  return (
    <div className="studio">
      ...
      <p className="muted">
        One essay in: the engine mines ideas, plans a calendar, then creates platform-native
        assets (4 versions each) that are humanized and quality-scored. Brand voice is injected
        automatically — assets are editable before you publish.
      </p>

      <label style={{ display: "block" }}>
        <span className="muted">Brand voice</span>
      ...
      <textarea
        className="big"
        value={essay}
        placeholder="Paste your essay / long source text here…"
      ...
      <div style={{ display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
        <label className="muted">
          Max ideas:&nbsp;
          <select value={maxIdeas} onChange={(e) => setMaxIdeas(Number(e.target.value))}>
            {[4, 6, 8, 10, 12].map((n) => <option key={n} value={n}>{n}</option>)}
          </select>
        </label>
        <label className="row muted">
          <input type="checkbox" checked={runMultiplier} onChange={(e) => setRunMultiplier(e.target.checked)} />
          Also run Multiplier (propose new IP)
        </label>
      </div>
      ...
      {result && (
        <div className="result">
          ...
          {result.multiplier_ip?.length > 0 && (
            <div className="result">
              <h3>New IP to spin up next</h3>
              <ul>
                {result.multiplier_ip.map((ip, i) => (
                  <li key={i}><strong>{ip.title}</strong> <span className="muted">({ip.format}) — {ip.why}</span></li>
                ))}
              </ul>
            </div>
          )}
          {Object.entries(groupByPlatform(result.assets)).map(([platform, list]) => (
            <div key={platform} className="result">
              ...
              {list.map((a) => {
                ...
                return (
                  <div className="card" key={a.id}>
                    ...
                    <span className="muted"> · {a.idea_ref}</span>
                    <span className="badge">★ {a.quality_score}/10</span>
                    <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
                      <button className="link" onClick={() => setOpenAsset(a)}>Edit / view</button>
                      <button className="link" onClick={() => onRepurpose(versions[0] || "")}>→ Repurpose</button>
                    </div>
                  </div>
                );
              })}
      ...
      {openAsset && (
        <div className="brief-detail">
          <button className="link" onClick={() => setOpenAsset(null)}>← Back to run</button>
          <h3>{openAsset.kind} · {openAsset.platform.replace("_", " ")}</h3>
          <p className="muted">from idea: {openAsset.idea_ref} · ★ {openAsset.quality_score}/10</p>
          <AssetEditor asset={openAsset} onSave={saveAsset} />
        </div>
      )}
    </div>
  );
...
function AssetEditor({ asset, onSave }) {
  const [versions, setVersions] = useState((asset.versions || []).map((v) => v || ""));

  function setV(i, val) {
    setVersions((cur) => cur.map((c, idx) => (idx === i ? val : c)));
  }

  async function save() {
    const next = await onSave(asset, versions);
    setVersions(next.map((v) => v || ""));
  }
    ...
    <div>
      {versions.map((v, i) => (
        <div key={i} style={{ marginBottom: 12 }}>
          <p className="muted">Version {i + 1}</p>
          <textarea
            className="big"
            value={v}
